...
//
//  this function mixes the semantic notion of a pizza object with the view (in html) of the object
//  I'd prefer to keep them distinct.
//
...
//----------------------------------------------------------------------------------------------------------------
// resizePizzas(size) is called when the slider in the "Our Pizzas6" section of the website moves.
// resizePizzas relies on a number of helper funcitions below
//
var resizePizzas = function(size) { 
    ...
    // 
    // Only called by changePizzaSizes(size) 
    //
    ...
    //
    // Only called immediately following this definition.
    //
    ...
    //
    // We could even have a SizeSelector class, an instance of which 
    // could hold the current size in memory as an attribute.
    // This would also allow for possibly changing the available sizes.
    // 
    ...
    // Big win by just calling querySelectorAll() only once and
    // moving the arithmetic out of the loop.  The elapsed time goes
    // from ~140ms to ~4ms
    //
    // (N.B. this assumes all of the pizzas are the same size.
    // That way we can just get the size of one of them, do the 
    // arithmetic, and update all the widths.)
    //
    // TODO: Is there a way to just update the style sheet?
    // Can we manipulate the CSSOM? If so perhaps we could 
    // move the operative 'width' attribute from the elements to the
    // CSS and just update it once.
    //
    // Other ideas:
    //  - Let width have an enumerated type, like ['100px', '200px', '300px'].
    //  - Keep the current width of the pizzas in a local var or OO-style
    //    attribute.  I could make this much cleaner if I rewrote it in coffeescript.
    //
    function changePizzaSizes(size) {

        var original_for_loop = false;
        if (original_for_loop) {
...
// TODO: JANK ALERT !
//
//  This is a LOT of pizzas.  How many are "above the fold". 
//  I don't know, but not 100.  Actually, all of the pizzas are below the fold.
//
...
//----------------------------------------------------------------------------------------------------------------
// Counter for number of times the pizzas in the background have scrolled.
// Used by updatePositions() to decide when to log the average time per frame
//
var frame = 0;

// Logs the average amount of time per 10 frames needed 
// to move the sliding background pizzas on scroll.
// Only called from updatePositions()
//
// arg: times is the array of User Timing measurements from updatePositions()
